...
const [username, qty] = inventoryItem;

const msg = addInventory(username, qty);

console.log('[Exercise 1.6]', msg);

function addInventory(username: string, quantity: number): string {
    return `Added ${quantity} ${username}s to inventory.`;
}